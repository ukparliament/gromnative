      ...
      Context("with a non-200 status code", func() {
        ...
        It("returns an error", func() {
          ...
          Expect(resp).To(Equal(&GetOutput{
            ...
            Error: "Recieved 500 status code from https://api.parliament.uk/query/person_by_id?person_id=43RHonMf: Error",
          }))

          Expect(err).To(HaveOccurred())
          Expect(err.Error()).To(Equal("Recieved 500 status code from https://api.parliament.uk/query/person_by_id?person_id=43RHonMf: Error"))
        })
      })
    })
